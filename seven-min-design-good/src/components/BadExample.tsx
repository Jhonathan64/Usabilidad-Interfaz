...
const products = [
  { id: 1, name: "AUDÍFONOS INALÁMBRICOS PREMIUM SUPER MEGA PRO MAX ULTRA DELUXE EDITION 2024", price: 99.99, priceText: "$99.99 USD DÓLARES AMERICANOS", img: headphonesImg },
  { id: 2, name: "MOCHILA", price: 149.99, priceText: "$149.99", img: backpackImg },
  { id: 3, name: "reloj bonito para la muñeca que da la hora y la fecha", price: 249.99, priceText: "$249.99 (IVA no incluido, o tal vez sí, no sabemos)", img: watchImg },
  { id: 4, name: "ZAPATILLAS", price: 119.99, priceText: "LLAME PARA PRECIO", img: sneakersImg },
];
        ...
        {products.map((product: any, i: number) => (
          <motion.div key={product.id} animate={{ rotate: [0, i % 2 === 0 ? 3 : -3, 0], scale: [1, 1.03, 0.97, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="border-4 p-3 relative cursor-pointer" onClick={() => onSelect(product.id)} style={{ background: `linear-gradient(${45 + i * 30}deg, ${flashColors[i % flashColors.length]}33, ${flashColors[(i + 2) % flashColors.length]}33)`, transform: `rotate(${(i - 1.5) * 4}deg)` }}>
            ...
            <div className="mt-2 flex flex-col gap-1">
              <motion.button onClick={(e: React.MouseEvent) => { e.stopPropagation(); addToCart(product.id); }} animate={{ background: ["#ff0000", "#00ff00", "#ff0000"] }} transition={{ duration: 0.5, repeat: Infinity }} className="w-full py-2 text-white text-xs font-bold" style={{ textShadow: "1px 1px 0 #000" }}>🔥 COMPRAR AHORA!!! 🔥</motion.button>
              <button onClick={(e) => { e.stopPropagation(); addToCart(product.id); }} className="w-full py-1 text-white text-xs" style={{ background: "#0000ff" }}>AGREGAR (agrega cantidad random)</button>
              <button className="w-full py-1 text-[8px]" style={{ background: "#8b00ff", color: "#ffff00" }}>¿QUÉ HACE ESTE BOTÓN? 🤷</button>
            </div>
          </motion.div>
        ))}
      </div>
